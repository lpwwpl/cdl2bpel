...
public class DefaultResourceProperties implements ResourceProperties{
	...
	public DefaultResourceProperties(org.eclipse.core.resources.IResource resource) {
		m_resource = resource;
	}
		...
		if (m_resource != null && property != null) {
			...
			if (property.equals(ResourceProperties.RESOURCE_LOCATION_PROPERTY)) {
				...
				if (ret == null && m_defaultResource != null) {
					...
					try {
						ret = (String)m_defaultResource.getPersistentProperty(key);
					} catch(Exception e) {
						logger.severe("Failed to retrieve persistent property '"+
								key+"' from default resource "+m_defaultResource);
					}
				}
			}
		}
    ...
    private static Logger logger = Logger.getLogger("edu.xjtu.cdl2bpel.common.resource");
