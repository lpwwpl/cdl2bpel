...
import edu.xjtu.cdl2bpel.utils.ToolTipComboBox;
...
public class ConversionUI extends JInternalFrame implements Provider {
	...
	private void showConversion() {
		...
		final TransformingPlugin algorithm = ((ConvertAlgorithm) algorithmList
	...
	private void jbInit() {
		...
		TransformingPluginCollection algorithmCollection = TransformingPluginCollection
				.getInstance();
		int selection = -1;
		...
		for (int i = 0; i < algorithmCollection.size(); i++) {
			TransformingPlugin algorithm = ((TransformingPlugin) algorithmCollection
					.get(i));
	...
	private ProvidedObjectComboItem[] getComboItems(TransformingPlugin plugin) {
		...
		for (int i = 0; i < po.length; i++) {
			if (plugin.accepts(po[i])) {
				accepted.add(new ProvidedObjectComboItem(
						"Framework Global Objects", po[i]));
				comboItems.add(po[i]);
			}
		}
...
class ConvertAlgorithm implements Comparable {
	private TransformingPlugin p;

	public ConvertAlgorithm(TransformingPlugin plugin) {
		p = plugin;
	}
