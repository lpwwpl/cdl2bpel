...
import edu.xjtu.cdl2bpel.models.BPELVisitable;
...
public class BPELProcess implements BPELVisitable {
	...
	public int countAllActivities() {
		return activity.countActivities("receive")
				+ activity.countActivities("assign")
				+ activity.countActivities("invoke")
				+ activity.countActivities("reply")
				+ activity.countActivities("while");

	}

	public double ComplexityOfActivies() {
		double count = 0;
		if (activity instanceof BPELStructured) {
			count = ((BPELStructured) activity).ComplexityOfActivity();
		} else
			count = 1;
		return count;
	...
	public int NumberOfEdges() {
		int number = 0;
		if (activity instanceof BPELStructured) {
			number = ((BPELStructured) activity).NumberOfEdges();
		} else
			number = 3;
		return number;
