...
public class JDOMAdapterNode {
	...
	/**
	 * Tricky toString which allows for copying entire elements and their
	 * children from the xml viewer.
	 * 
	 * @return String
	 */
	@Override
	public String toString() {
		...
		if (childCount() > 0) {
			...
			sb.append(tab + node.getName() + "[" + node.getTextTrim() + "]"
					+ lf);
		}
	...
	public String toString(int r) {
		...
		if (childCount() > 0) {
			...
			sb.append(space + node.getName() + "[" + node.getTextTrim() + "]"
					+ lf);
		}
